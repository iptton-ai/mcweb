...
//   塔内粘液垫连跳是捷径/彩蛋通路（观星台星辉门+钻石），塔外螺旋楼梯（逐格+1，顶端到
//   平台高度）是主路，塔顶西沿门墙锁③（数学·周长两步推理，答题机在门旁护栏上、楼梯顶
//   可按）→ 东侧跳台跳下「云池」（2 格深水池缓冲）→ 终点岛（蹦床+奖杯）。
// 主通路（3 锁/2 检查点/终点）全部 6 邻步行·跑跳可达；校验器不懂弹跳，粘液只出现在
// 校验盲区（塔内井与终点蹦床），且塔内井各层不构成任何锁的绕行通道。
// 锁③原设计把答题机嵌在北沿护栏、门在其西侧：门柱挡死楼梯方向的视线/触及，
// 答题机根本按不到=关卡不可通关（2026-09-16 修复：门移到西沿护栏缺口=楼梯顶正对，
// 北沿护栏补全 + 门洞上方与护栏顶加封板，杜绝楼梯顶斜跳上护栏翻入平台）。
...
export function buildLevel() {
    ...
    // ---- E 弹跳塔岛 ----
    island(44, 14, 50, 26);
    flagAt(c, 43, T, 20, FLAG_CHECKPOINT); // 检查点②：石板桥东端（摔下来回这里）
    torchPost(c, 45, T, 15); torchPost(c, 50, T, 15); torchPost(c, 44, T, 25); torchPost(c, 50, T, 24);
    ...
    // ---- 塔外螺旋楼梯（主路，逐格 +1）：西壁一列向北，顶端到平台高度（feet T+9），
    //      正对西沿护栏缺口的锁③门——门是唯一入口（旧版止步平台下 1 格且门在北沿，
    //      答题机被门柱挡死按不到，2026-09-16 修复）----
    const steps = [
        [44, T + 1, 24], [44, T + 2, 23], [44, T + 3, 22], [44, T + 4, 21],
        [44, T + 5, 20], [44, T + 6, 19], [44, T + 7, 18], [44, T + 8, 17],
    ];
    for (const [sx, sy, sz] of steps) c.set(sx, sy, sz, COBBLESTONE);
    // 最后一级=平台西沿缺口正西，平台在 y=T+8（x45..51 / z16..23，东探出 1 格压住云池护墙）
    c.fill(45, T + 8, 16, 51, T + 8, 23, COBBLESTONE);
    // 平台护栏（1 高）：西 z18..22 / 北 x45..50（整段封死）/ 南 x45..51 / 东 z16..18 与
    //   z22..23；留两个口：西 z17=锁③门、东 z19..21=跳台口（跳云池）
    c.fill(45, T + 9, 18, 45, T + 9, 22, COBBLESTONE);
    c.fill(45, T + 9, 16, 50, T + 9, 16, COBBLESTONE);
    c.fill(45, T + 9, 23, 51, T + 9, 23, COBBLESTONE);
    c.fill(51, T + 9, 16, 51, T + 9, 18, COBBLESTONE);
    c.fill(51, T + 9, 22, 51, T + 9, 23, COBBLESTONE);
    // 门墙锁③（数学·周长两步推理）：门洞在西沿护栏缺口 z=17（楼梯顶正对、同高可走入），
    // 答题机嵌门旁护栏（南邻）；门洞上方封两级防跳门
    doorway(c, { x: 45, z: 17, y0: T + 9, facing: 3 }, { x: 45, y: T + 9, z: 18 });
    // 檐板：盖住门洞与西沿/北沿护栏顶——楼梯顶与平台同高后，护栏顶（feet T+10）从楼梯
    // 一步斜跳可及，不封板就能踩护栏翻进平台绕过锁③
    c.fill(45, T + 11, 16, 45, T + 11, 22, COBBLESTONE);
