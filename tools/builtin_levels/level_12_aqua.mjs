...
//   空气室，锁① science·水的性质，检查点；潜水井=室底 1×2 水口 (29,5..6,17..18)，开在
//   东墙门北侧：沿轴光带（甬道口→中段→衔接→轴端→井底 6 颗夜明珠）把玩家牵到尽头，
//   井口两侧楼板嵌灯框出亮水洞——否则水层里只看得见玻璃楼板后的门与答题机
//   （右键全命中玻璃），找不到入口）→
...
export function buildLevel() {
    ...
    // 潜水井光带（2026-09-16 玩家反馈「答题的门被玻璃墙封住/入口太不明显」）：
    // 室底水层一路很暗，沿轴铺发光地砖把玩家「牵」到尽头——井底正下方 +
    // 轴线→井的过渡格各一颗；没有这些光，玩家只看得见玻璃楼板后面的门与
    // 答题机（右键全命中玻璃），根本发现不了楼板上的入口
    c.set(15, 2, 20, lampId(1));  // 甬道口（跳水点正下方池底）
    c.set(20, 2, 20, lampId(1));  // 甬道中段
    c.set(24, 2, 20, lampId(1));  // 甬道→气室底水层衔接
    c.set(27, 2, 20, lampId(1));  // 轴线末端
    c.set(29, 2, 19, lampId(1));  // 轴线→井过渡
    c.set(29, 2, 18, lampId(1));  // 井柱正下方
    ...
    // 潜水井（东墙、门北侧，1×2 水口 z17..18）：室底水层尽头的发光井柱。
    // 刻意不放在 z=20 走廊线上（井口在门前必经线上=1×2 坑，走向门会掉回去）；
    // 扩成 1×2 让入井容错翻倍、楼板洞更醒目。升井向南爬出，答题机就在手边
    c.fill(29, 5, 17, 29, 6, 18, WATER);
    // 井口两侧楼板各嵌一颗夜明珠：从下方水层仰望=两盏灯夹着一个亮水洞，
    // 从室内看=井口被灯框出来
    c.set(28, 5, 18, lampId(1));
    c.set(30, 5, 18, lampId(1));
    // 室内珊瑚与夜明珠
    c.set(25, 6, 22, WOOL); c.set(25, 7, 22, WOOL);
    c.set(29, 6, 23, WOOL);
    for (const [fx, fz] of [[26, 6, 17], [28, 6, 23]]) c.set(fx, 6, fz, FLOWER);
    c.set(28, 6, 16, lampId(1)); // 原 (29,6,17) 让位井口，西北移一格
    c.set(28, 6, 18, TORCH);
