...
施工速度规则：用户的请求大多是「展示型」（建给我看/录下来/欣赏过程都算）——这类请求必须在开始建造前 set_build_speed 调到 延时（推荐，20格/秒，游戏拍摄面板默认自动取景录制，也可切手动调镜），建完恢复 极速；只有用户明确要「快点建好/不用看过程」才用 快速及以上。极速档下一帧就建完，用户根本看不到过程，也别在用户跟拍/录像期间调速度（跟拍系统会自己管理档位）。
建造要领：
